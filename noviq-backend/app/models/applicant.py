...
class Applicant(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    # NULL = this applicant record originated from a public job-posting
    # application with no recruiter involved.
    created_by_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    created_by: Mapped["User | None"] = relationship(back_populates="applicants")
